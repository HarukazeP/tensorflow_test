...
def my_clean(sentence, vocab_set, subword_option):
  """Clean and handle BPE or SPM outputs."""
  #sentence = sentence.strip()   #空白と改行を削除？
  sentence=re.sub(r'[^a-z{}<> ]', '', sentence)
  sentence=change_unk(sentence, vocab_set)
...
def bleu(ref_file, trans_file, vocab_set, subword_option=None):
  ...
  for references in zip(*reference_text):
    reference_list = []
    for reference in references:
      reference = my_clean(reference, vocab_set, subword_option)
  ...
  with codecs.getreader("utf-8")(tf.gfile.GFile(trans_file, "rb")) as fh:
    for line in fh:
      line = my_clean(line, vocab_set, subword_option=None)
...
def count_correct_num(ref_path, ans_path, vocab_set):
    cloze_sent_num=0
    all_correct_num=0
    cloze_correct_num=0
    sent_correct_num=0
...
def print_result(ref_path, ans_path, vocab_set):
    print('file: ',ref_path, 'changed to unk')
    BLEU=bleu(ref_path, ans_path, vocab_set)
    line, num_cloze, allOK, cloze, sent =count_correct_num(ref_path, ans_path, vocab_set)
    print('\nfile: ',ref_path, 'changed to unk')
