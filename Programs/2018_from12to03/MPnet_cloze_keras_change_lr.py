...
def build_model(vocab_size, emb_size, hidden_size, emb_matrix, my_model_kind):
    ...
    opt=optimizers.Adam(lr=0.0001, clipnorm=math.sqrt(5))    #デフォルト：lr=0.001
...
if __name__ == '__main__':
    ...
    #ループして複数モデル学習放置用
        ...
        # 2.モデル定義
        models=['origin', 'plus_KenLM', 'plus_both' , 'replace_KenLM', 'replace_both', 'plus_CAR', 'replace_CAR', ]
        ...
        for my_model_kind in models:
            ...
            #モデルとか結果とかを格納するディレクトリの作成
            save_path=save_path+'_MPNet_change_lr_'+my_model_kind
            if os.path.exists(save_path)==False:
                os.mkdir(save_path)
            save_path=save_path+'/'
