...
'''
MPNetとKenLMで正答した問題や間違えた問題の分布？割合？を確認する用
テスト専用
...
# 4.モデルによる予測
###########################
def model_test_MPNet_and_KenLM(ngram, lang, MPNet_model, KenLM_model_path, cloze_path, choices_path, ans_path, my_model_kind, data_name=''):
    print(data_name)
    use_Ng, use_AR, use_KenLM, use_CAR=use_config(my_model_kind)
    ...
    KenLM_pred=Ngram2(KenLM_model).get_KenLM_score2(test_X, test_C)
    both_OK=0
    only_M_OK=0
    only_K_OK=0
    both_NG=0
