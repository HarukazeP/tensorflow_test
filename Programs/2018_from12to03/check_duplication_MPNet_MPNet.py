...
'''
MPNetどうしで正答した問題や間違えた問題の分布？割合？を確認する用
テスト専用
...
KenLA_path='/media/tamaki/HDCL-UT/tamaki/M2/kenlm_models/all_words/'
...
class Ngram():
    def __init__(self):
        ...
        self.KenLM_1gram =''
        #kenlm.LanguageModel(KenLA_path+'CLOTH_NUM_N1.arpa')
        self.KenLM_2gram = kenlm.LanguageModel(KenLA_path+'CLOTH_NUM_N2.arpa')
        self.KenLM_3gram = kenlm.LanguageModel(KenLA_path+'CLOTH_NUM_N3.arpa')
        self.KenLM_4gram = kenlm.LanguageModel(KenLA_path+'CLOTH_NUM_N4.arpa')
        self.KenLM_5gram = kenlm.LanguageModel(KenLA_path+'CLOTH_NUM_N5.arpa')
    #前処理
...
# 4.モデルによる予測
###########################
def model_test_MPNet_and_MPNet(ngram, lang, model_A, model_B, cloze_path, choices_path, ans_path, model_kind_A, model_kind_B, data_name=''):

    print(data_name)
    #model_A用
    use_Ng, use_AR, use_KenLM, use_CAR=use_config(model_kind_A)
    ...
    A_pred=model_A.predict(X_test, batch_size=data_size, verbose=1)


    #model_B用
    use_Ng, use_AR, use_KenLM, use_CAR=use_config(model_kind_B)
    ...
    B_pred=model_B.predict(X_test, batch_size=data_size, verbose=1)
    ...
    print('only_A_OK: %d (%.4f)' % (only_A_OK, 1.0*only_A_OK/line))
    print('only_B_OK: %d (%.4f)' % (only_B_OK, 1.0*only_B_OK/line))
    print('both_NG  : %d (%.4f)' % (both_NG, 1.0*both_NG/line))
...
if __name__ == '__main__':
    ...
    #model_A.load_weights(save_path+args.model+'.hdf5')
